...
- **プロンプトは具体的に**。ハンズオンでは再現性を優先し、「完成形」「機能」「見た目」「制約」「確認方法」を1つの指示に含める。エージェントの自律性を見せるより、参加者が同じゴールに到達できることを優先する。
- **ハンズオンは ToDo アプリ**。無料モデルでも完走しやすいように `index.html` 1ファイル完結で作る。ブラウザで即確認でき、localStorage の保存も体験できる。
...
- 無料・軽量モデルでは複数ファイル生成が途中で崩れることがあるため、ハンズオン本編では単一HTMLにする。
